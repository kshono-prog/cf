...
export function GasSupportCard(props: {
  ...
  const claimAmt = gas?.claimableAmountPol ?? "0.02";
  const faucetAddress = gas?.faucetAddress;

  return (
    <div className="card p-4 space-y-2 bg-white">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold">ガス代支援（POL）</h2>
        {gasLoading && (
      ...
      <p className="text-xs text-gray-600">
        JPYCを100円以上保有し、POL残高が0の方は、少額のPOLを受け取って初回の送金を始められます。
      </p>

      {gas && (
        <div className="text-[11px] text-gray-700 space-y-1">
          <div>JPYC残高：{gas.jpycBalance ?? "-"}</div>
          <div>POL残高：{gas.polBalance ?? "-"}</div>
          <div>受取予定：{claimAmt} POL</div>
        </div>
      )}
      ...
      >
        {gasClaiming ? "受取処理中..." : `${claimAmt} POL を受け取る`}
      </button>

      {faucetAddress && (
        <div className="pt-2 border-t border-gray-200 space-y-1">
          <p className="text-xs text-gray-600">
            この機能を支援する（POL寄付 / Faucet原資）
          </p>
          <p className="text-xs font-mono break-all">{faucetAddress}</p>
          {gas?.faucetBalancePol && (
            <p className="text-[11px] text-gray-500">
              Faucet残高：{gas.faucetBalancePol} POL
            </p>
          )}
        </div>
      )}
