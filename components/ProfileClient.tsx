...
function getPublicClientForChain(chainId: number) {
  if (chainId === 137 && process.env.NEXT_PUBLIC_POLYGON_RPC_URL) {
  ...
  if (chainId === 80002 && process.env.NEXT_PUBLIC_POLYGON_AMOY_RPC_URL) {
    return createPublicClient({
      chain: polygonAmoy,
  ...
  if (chainId === 43114 && process.env.NEXT_PUBLIC_AVALANCHE_RPC_URL) {
    return createPublicClient({
      chain: avalanche,
  ...
  if (chainId === 43113 && process.env.NEXT_PUBLIC_AVALANCHE_FUJI_RPC_URL) {
    return createPublicClient({
      chain: avalancheFuji,
...
// JPYC アドレスは tokenRegistry から取得するため、ここでは固定値を持たない
...
type ProjectProgressGet = {
  ...
  goalCheck: {
    targetAmountJpyc: number | null;
    targetAmountDecimal18: string | null;
...
export default function ProfileClient({
  ...
  const REQUIRED_CHAIN: SupportedChainId = getDefaultChainId();
  const requiredChainConfig = getChainConfig(REQUIRED_CHAIN);
  ...
  const onWrongChain =
    ...
    !isNetworkMatched({
      ...
      requiredChainId: REQUIRED_CHAIN,
    });
  ...
  async function resumeAfterReturnFromWallet() {
    ...
    try {
      ...
      const publicClient = getPublicClientForChain(last.chainId);
      if (!publicClient) {
  ...
  /* 目標進捗 JPYC 残高 (creator.address) - 既存ロジックを readBalances ベースに変更 */
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [creator.address, creator.goalTargetJpyc]);

  /* 接続中ウォレットの ネイティブ / JPYC 残高（受取チェーン） */
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [connected, activeAddress, onWrongChain, REQUIRED_CHAIN]);

  /* 過去24時間の JPYC 投げ銭合計（オンチェーン） */
  useEffect(() => {
    if (
      !publicClient ||
      !connected ||
    ...
    (async () => {
      try {
        ...
        const approxBlocksPerDay = 43200;
        const fromBlock = BigInt(Math.max(latest - approxBlocksPerDay, 0));
        const toBlock = latestBlock;

        const jpycOnChain = getTokenOnChain("JPYC", REQUIRED_CHAIN);
        if (!jpycOnChain) {
  ...
  async function switchChain() {
    ...
    const cfg = requiredChainConfig;
    if (!cfg) return;

    const chainHex = `0x${cfg.id.toString(16)}`;
    const rpcUrl =
      cfg.viemChain.rpcUrls.default?.http[0] ??
      cfg.viemChain.rpcUrls.default?.http[0] ??
      "";
  ...
  async function send(overrideAmount?: string) {
    try {
      ...
      if (onWrongChain) {
        alert(
          "ネットワークを切り替えてください（上部のネットワークボタンから変更できます）"
        );
        return;
      }
      if (!toAddress) {
      ...
      const tokenOnChain = getTokenOnChain(tokenKey, REQUIRED_CHAIN);
      if (!tokenOnChain) {
      ...
      saveLastTx({
        ...
        chainId: currentChainId ?? REQUIRED_CHAIN,
        currency,
        amount: amtStr,
        toAddress,
  ...
  function WalletDebug() {
    ...
    return (
      <pre className="mt-4 text-[10px] bg-gray-100 p-2 rounded overflow-auto">
        {JSON.stringify(
          {
            accountStatus: acct.status,
            address: acct.address ?? null,
            chainId: acct.chainId ?? null,
            connectStatus: conn.status,
            connectError: conn.error?.message ?? null,
          },
          null,
          2
        )}
              ...
              <div className="mt-3 rounded-xl border border-amber-300/60 bg-amber-50/80 dark:border-amber-300/80 dark:bg-amber-50/80 p-3 text-amber-800">
                <div className="flex items-start justify-between gap-3">
                  <div className="text-xs sm:text-sm">
                    ネットワークが違います。指定のネットワークに切り替えてください。
                  </div>
