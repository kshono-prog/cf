/* app/api/projects/[projectId]/reward-tiers/[tierId]/start-production/route.ts */
...
export async function POST(req: NextRequest, ctx: { params: Promise<Params> }) {
  try {
    ...
    // readiness を再計算し、NOT_STARTED → READY_TO_START を自動反映した上で判定する。
    ...
    if (progress.productionStatus !== "READY_TO_START") {
      return errJson("PRODUCTION_NOT_READY", 409);
    }
    ...
    const nextProgress = buildRewardTierProgressDto({
      tier: updated,
      confirmedSupportCount,
      confirmedSupportAmountJpyc,
    });
    ...
    return okJson({
      ok: true,
      productionStatus: "IN_PROGRESS",
      tier: serializeRewardTierWithProgress({
        tier: updated,
    ...
    console.error("REWARD_TIER_START_PRODUCTION_FAILED", e);
    return errJson("REWARD_TIER_START_PRODUCTION_FAILED", 500);
  }
}
