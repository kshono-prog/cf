...
import { getCreatorManagerFeed } from "@/lib/operations/managerFeed";
import {
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const managerFeed = await getCreatorManagerFeed({
      creatorProfileId,
      limit: toLimit(searchParams.get("limit"), 4),
    });

    return okJson(managerFeed);
  } catch (error) {
    console.error("MYPAGE_MANAGER_FEED_GET_FAILED", error);
    return errJson("MYPAGE_MANAGER_FEED_GET_FAILED", 500);
  }
}
