  ...
  const restoreFetch = installFetchStub(async (input) => {
    ...
    if (url.includes("/api/owner-auth/nonce")) {
      ...
      return okJson({
        ...
        message: "sign this",
        expiresAt: "2099-03-21T01:00:00.000Z",
      });
    }
    if (url.endsWith("/api/owner-auth/session")) {
      ...
      return okJson({
        ok: true,
        address: OWNER_ADDRESS,
        expiresAt: "2099-03-21T02:00:00.000Z",
      });
    }
...
test("ownerAuthFetch GET invalidates cached viewer identity after a 401 session response", async () => {
  resetOwnerAuthClientState();
...
test("ownerAuthFetch POST retries auto-auth requests after a 401", async () => {
  resetOwnerAuthClientState();
